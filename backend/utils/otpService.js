...
class OTPService {
  ...
  async getOTPStatus(email) {
    try {
      ...
      const data = await redisService.getJSON(otpKey);
      if (!data) {
        return {
          exists: false,
          expired: false,
      ...
      const now = Date.now();
      const isExpired = now > data.expiryTime;
      ...
      return {
        ...
        remainingTimeSeconds: isExpired
          ? 0
          : Math.floor((data.expiryTime - now) / 1000),
      };
    } catch {
      return null;
    }
  ...
  async resendOTP(email, expiresInMinutes = 10) {
    const status = await this.getOTPStatus(email);
    if (status && !status.expired) {
      const waitMinutes = Math.ceil((status.expiresAt - Date.now()) / 60000);
      return {
        success: false,
        message: `Wait ${waitMinutes} minutes before requesting a new OTP`,
      };
    }
    const otp = this.generateOTP();
    await this.storeOTP(email, otp, expiresInMinutes);
    return { success: true, message: "New OTP generated", otp };
  }

  async rateLimitCheck(email, windowMinutes = 60, maxRequests = 5) {
    if (!this.useRedis) return { allowed: true };
    try {
