...
export const getVerificationRequestById = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // If data not in cache OR cached data doesn't have proper instructor structure
    if (
      !requestData ||
      !requestData.instructor ||
      !requestData.instructor.user
    ) {
      console.log("Fetching from database - cache miss or incomplete data");
...
export const getVerificationStats = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const levelCounts = requestsByLevel.reduce(
      (acc, item) => {
        acc[item.verificationLevel.toLowerCase()] =
          item._count.verificationLevel;
        return acc;
      },
      {
        basic: 0,
    ...
    const verificationDistribution = verificationsByLevel.reduce(
      (acc, item) => {
    ...
    const stats = {
      overview: {
        ...
        underReviewRequests: statusCounts.under_review,
        totalInstructors,
        verifiedInstructors,
        verificationRate:
          totalInstructors > 0
            ? ((verifiedInstructors / totalInstructors) * 100).toFixed(2)
            : "0.00",
      },
      byLevel: levelCounts,
    ...
    await redisService.setJSON(cacheKey, stats, { ex: 180 });
